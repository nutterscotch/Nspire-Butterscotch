...
// The cooked atlas stores each sprite at 1/2 the engine's native pixel size — that
// constant is the preprocessor's bargain. The view→fb scale combines this atlas
// halving with the engine's port/view ratio so a 320-wide Undertale room and a
// 640-wide window-coord battle GUI both end up filling our 320-px framebuffer.
...
// Convert an engine x/y to framebuffer pixel coords using the active view mapping.
...
static inline NspireEngineRenderer* as_ner(Renderer* r) { return (NspireEngineRenderer*) r; }
static void ns_bindTarget(NspireEngineRenderer* ner, int32_t id);

// Pick the right CLUT for an atlas (4bpp vs 8bpp lookup tables).
static const uint16_t* clut_for(NspireEngineRenderer* ner, NspireAtlas* atlas, uint16_t clutIndex) {
    if (atlas->bpp == 8) {
        if (clutIndex >= (uint16_t) ner->assets->clut8Count) return NULL;
        return ner->assets->clut8[clutIndex].entries;
    } else {
        if (clutIndex >= (uint16_t) ner->assets->clut4Count) return NULL;
...
static void ns_beginFrame(Renderer* r, int32_t gameW, int32_t gameH, int32_t windowW, int32_t windowH) {
...
static void ns_endFrame(Renderer* r) {
    ...
    // scene's working set — preloading/keeping all 14 1024px atlases is ~9.5 MB and
    // crash-restarts alongside the 16 MB data.win. Eviction was wrongly suspected of
    // the battle-font garble; the real cause was the 512-atlas resize bug (now fixed
    // in the cook), so per-frame eviction is safe again.
    if (ner->assets) NspireAssets_releaseUntouchedAtlases(ner->assets);
}

static void ns_beginView(Renderer* r, int32_t viewX, int32_t viewY, int32_t viewW, int32_t viewH,
                         int32_t portX, int32_t portY, int32_t portW, int32_t portH, float viewAngle) {
    NspireEngineRenderer* ner = as_ner(r);
    (void) viewAngle;

    // The view describes "what to look at" in room coords; the port describes "where to
    // paint that on the platform's output buffer." Since our framebuffer is HR_SCALE_ATLAS
    // of the engine's nominal port pixels, the engine-to-fb scale folds both factors.
    float scaleX = (viewW > 0) ? ((float) portW / (float) viewW) * HR_SCALE_ATLAS : HR_SCALE_ATLAS;
    float scaleY = (viewH > 0) ? ((float) portH / (float) viewH) * HR_SCALE_ATLAS : HR_SCALE_ATLAS;
    ...
    if (sx < 0) sx = 0;
    if (sy < 0) sy = 0;
    if (sx + sw > NSPIRE_FB_W) sw = NSPIRE_FB_W - sx;
    if (sy + sh > NSPIRE_FB_H) sh = NSPIRE_FB_H - sy;
    if (sw < 0) sw = 0;
    if (sh < 0) sh = 0;
    ner->fb.scissorX = sx;
    ner->fb.scissorY = sy;
    ner->fb.scissorW = sw;
    ner->fb.scissorH = sh;
    ner->portX = portX; ner->portY = portY; ner->portW = portW; ner->portH = portH;
    ner->lastViewW = viewW; ner->lastViewH = viewH;
    ner->lastPortX = portX; ner->lastPortY = portY;
}

static void ns_endView(Renderer* r) { (void) r; }

static void ns_beginGUI(Renderer* r, int32_t guiW, int32_t guiH, int32_t portX, int32_t portY, int32_t portW, int32_t portH) {
...
//
// As a side effect, marks the underlying atlas as touched-this-frame and demand-loads
// its pixel buffer if not currently resident. Returns NULL if the atlas can't be loaded
// (out-of-memory, file error) so the caller skips the draw silently.
static NspireTpagEntry* lookup_tpag(NspireEngineRenderer* ner, int32_t tpagIndex) {
    if (!ner->assets) return NULL;
    if (tpagIndex < 0 || tpagIndex >= ner->assets->tpagCount) return NULL;
    NspireTpagEntry* t = &ner->assets->tpags[tpagIndex];
    if (t->atlasId == 0xFFFF) return NULL;
    if (t->cropW == 0 || t->cropH == 0) return NULL;
    if ((int32_t) t->atlasId >= ner->assets->atlasCount) return NULL;
    if (!NspireAssets_ensureAtlas(ner->assets, (int32_t) t->atlasId)) return NULL;
    return t;
}
...
// divide-free (v*(t+1))>>8 modulate (exact identity at t=255; ARM926 has no FPU).
// Cached on (src,colour,bpp) in `ner` and reset each frame: a tiled tinted
// background hits this hundreds of times/frame with identical args, so the
// O(256) rebuild collapses to one and the rest are pointer-equal cache hits.
static const uint16_t* tint_palette(NspireEngineRenderer* ner,
                                    const uint16_t* src, uint8_t bpp,
                                    uint32_t color) {
    if ((color & 0x00FFFFFFu) == 0x00FFFFFFu) return src;
    ...
    ner->tintSrc = src; ner->tintColor = color; ner->tintBpp = bpp;
    return ner->tintPal;
}

static void ns_drawSprite(Renderer* r, int32_t tpagIndex,
    ...
    // Atlas-to-fb stretch: cropW/cropH are atlas pixels (= engine pixels * HR_SCALE_ATLAS).
    // Final fb size = engineW * xscale * viewToFb. Atlas size on disk = engineW * HR_SCALE_ATLAS.
    // Therefore stretchAtlas = xscale * viewToFb / HR_SCALE_ATLAS.
    float stretchX = ax * ner->viewToFbScaleX / HR_SCALE_ATLAS;
    float stretchY = ay * ner->viewToFbScaleY / HR_SCALE_ATLAS;

    // Rotated path (Sans's Gaster Blasters aim by rotating image_angle). Map the
    // crop region through an affine that rotates about the sprite's origin point,
    // which lands at the engine (x,y) -> framebuffer pivot. Consistent with the
    // axis-aligned path below when angle == 0.
    if (angleDeg != 0.0f && angleDeg != 360.0f) {
        ...
        // Crop-local position of the sprite origin (atlas px = engine px * HR_SCALE).
        ...
        NspireRenderer_drawSpriteAffine(&ner->fb, atlas->pixels, atlas->stride, atlas->bpp,
            t->atlasX, t->atlasY, t->cropW, t->cropH, palette,
            ox, oy, Ux, Uy, Vx, Vy, alpha255);
        return;
    }

    int32_t destX = engine_to_fb_x(ner, x - originX * ax) + (int32_t) ((float) t->cropX * stretchX);
    int32_t destY = engine_to_fb_y(ner, y - originY * ay) + (int32_t) ((float) t->cropY * stretchY);

    // Identity fast path (atlas pixel = framebuffer pixel).
    if (stretchX > 0.999f && stretchX < 1.001f && stretchY > 0.999f && stretchY < 1.001f) {
        if (doBlend) {
            if (atlas->bpp == 8) {
                NspireRenderer_drawSpritePart8Blend(&ner->fb, atlas->pixels, atlas->stride,
                    t->atlasX, t->atlasY, t->cropW, t->cropH, palette, destX, destY, alpha255);
            } else {
                NspireRenderer_drawSpritePart4Blend(&ner->fb, atlas->pixels, atlas->stride,
                    t->atlasX, t->atlasY, t->cropW, t->cropH, palette, destX, destY, alpha255);
            }
        } else if (atlas->bpp == 8) {
            NspireRenderer_drawSpritePart8(&ner->fb, atlas->pixels, atlas->stride,
                t->atlasX, t->atlasY, t->cropW, t->cropH, palette, destX, destY);
        } else {
    ...
    int32_t destW = (int32_t) ((float) t->cropW * stretchX);
    int32_t destH = (int32_t) ((float) t->cropH * stretchY);
    if (destW < 1 || destH < 1) return;

    if (doBlend) {
        if (atlas->bpp == 8) {
            NspireRenderer_drawSpritePart8StretchedBlend(&ner->fb, atlas->pixels, atlas->stride,
                t->atlasX, t->atlasY, t->cropW, t->cropH, palette, destX, destY, destW, destH, alpha255);
        } else {
            NspireRenderer_drawSpritePart4StretchedBlend(&ner->fb, atlas->pixels, atlas->stride,
                t->atlasX, t->atlasY, t->cropW, t->cropH, palette, destX, destY, destW, destH, alpha255);
        }
    } else if (atlas->bpp == 8) {
        NspireRenderer_drawSpritePart8Stretched(&ner->fb, atlas->pixels, atlas->stride,
            t->atlasX, t->atlasY, t->cropW, t->cropH, palette, destX, destY, destW, destH);
    } else {
        NspireRenderer_drawSpritePart4Stretched(&ner->fb, atlas->pixels, atlas->stride,
            t->atlasX, t->atlasY, t->cropW, t->cropH, palette, destX, destY, destW, destH);
    }
}

static void ns_drawSpritePart(Renderer* r, int32_t tpagIndex,
    ...
    NspireTpagEntry* t = lookup_tpag(ner, tpagIndex);
    if (!t) return;
    (void) pivotX; (void) pivotY; (void) alpha;
    gNspireDrawStats.sprCalls++;
    gNspireDrawStats.sprPx += (uint32_t) t->cropW * (uint32_t) t->cropH;
    ...
    // Rotated sub-rect: rotate about the engine (x,y) draw anchor.
    if (angleDeg != 0.0f && angleDeg != 360.0f) {
        float rad = -angleDeg * 3.14159265358979f / 180.0f;
        float c = cosf(rad), sn = sinf(rad);
        float sgnX = xscale < 0 ? -1.0f : 1.0f;
        ...
        NspireRenderer_drawSpriteAffine(&ner->fb, atlas->pixels, atlas->stride, atlas->bpp,
            sampleX, sampleY, iW, iH, palette, ox, oy, Ux, Uy, Vx, Vy, 255);
        return;
    }

    int32_t destX = engine_to_fb_x(ner, x) + (int32_t) ((float) (iX - hSrcX) * stretchX);
    int32_t destY = engine_to_fb_y(ner, y) + (int32_t) ((float) (iY - hSrcY) * stretchY);

    if (stretchX > 0.999f && stretchX < 1.001f && stretchY > 0.999f && stretchY < 1.001f) {
        if (atlas->bpp == 8) {
            NspireRenderer_drawSpritePart8(&ner->fb, atlas->pixels, atlas->stride,
                sampleX, sampleY, iW, iH, palette, destX, destY);
        } else {
            NspireRenderer_drawSpritePart4(&ner->fb, atlas->pixels, atlas->stride,
                sampleX, sampleY, iW, iH, palette, destX, destY);
        }
    ...
    int32_t destW = (int32_t) ((float) iW * stretchX);
    int32_t destH = (int32_t) ((float) iH * stretchY);
    if (destW < 1 || destH < 1) return;

    if (atlas->bpp == 8) {
        NspireRenderer_drawSpritePart8Stretched(&ner->fb, atlas->pixels, atlas->stride,
            sampleX, sampleY, iW, iH, palette, destX, destY, destW, destH);
    } else {
        NspireRenderer_drawSpritePart4Stretched(&ner->fb, atlas->pixels, atlas->stride,
            sampleX, sampleY, iW, iH, palette, destX, destY, destW, destH);
    }
...
static void ns_drawSpritePos(Renderer* r, int32_t tpagIndex,
                             float x1, float y1, float x2, float y2,
                             float x3, float y3, float x4, float y4, float alpha) {
    NspireEngineRenderer* ner = as_ner(r);
    (void) x3; (void) y3; // BR derived from the parallelogram (no perspective)
    NspireTpagEntry* t = lookup_tpag(ner, tpagIndex);
    if (!t) return;
    gNspireDrawStats.sprCalls++;
    gNspireDrawStats.sprPx += (uint32_t) t->cropW * (uint32_t) t->cropH;
    NspireAtlas* atlas = &ner->assets->atlases[t->atlasId];
    const uint16_t* palette = clut_for(ner, atlas, t->clutIndex);
    if (!palette || alpha <= 0.0f) return;

    // Corners: 1=TL 2=TR 4=BL. Map to fb, build the affine that sends the crop
    // region's (0,0)->TL, (cropW,0)->TR, (0,cropH)->BL.
    float tlx = (float) engine_to_fb_x(ner, x1), tly = (float) engine_to_fb_y(ner, y1);
    float trx = (float) engine_to_fb_x(ner, x2), try_ = (float) engine_to_fb_y(ner, y2);
    float blx = (float) engine_to_fb_x(ner, x4), bly = (float) engine_to_fb_y(ner, y4);
...
static inline void ns_rectSpan(NspireEngineRenderer* ner, int32_t x, int32_t y,
                               int32_t w, int32_t h, uint16_t c565, float alpha) {
    if (w <= 0 || h <= 0 || alpha <= 0.0f) return;
    if (alpha >= 0.996f) {
        NspireRenderer_fillRect(&ner->fb, x, y, w, h, c565);
    } else {
        NspireRenderer_blendRect(&ner->fb, x, y, w, h, c565, (uint8_t) (alpha * 255.0f));
    }
}

// Shared by draw_rectangle / draw_rectangle_color. GameMaker's `outline` flag
// draws only the 1px border — Undertale's attack telegraphs (Sans bone warnings,
// Asriel's box/star attacks) rely on this to mark where a hit will land WITHOUT
// occluding the play area. Filling it instead (the old `(void) outline`) turned
// every telegraph into a solid box. Border thickness tracks the view->fb scale
// so it stays visible at any scale (like ns_drawLine's pen width).
static void ns_fillOrOutlineRect(NspireEngineRenderer* ner,
                                 float x1, float y1, float x2, float y2,
    ...
    if (hx2 < hx1) { int32_t t = hx1; hx1 = hx2; hx2 = t; }
    if (hy2 < hy1) { int32_t t = hy1; hy1 = hy2; hy2 = t; }
...
static void ns_drawRectangle(Renderer* r, float x1, float y1, float x2, float y2, uint32_t color, float alpha, bool outline) {
    ns_fillOrOutlineRect(as_ner(r), x1, y1, x2, y2, color, alpha, outline);
}
static void ns_drawRectangleColor(Renderer* r, float x1, float y1, float x2, float y2, uint32_t c1, uint32_t c2, uint32_t c3, uint32_t c4, float alpha, bool outline) {
...
static void ns_drawLine(Renderer* r, float x1, float y1, float x2, float y2, float width, uint32_t color, float alpha) {
    NspireEngineRenderer* ner = as_ner(r);
    if (alpha <= 0.0f) return;
    uint8_t b = (uint8_t) ((color >> 16) & 0xFF);
    uint8_t g = (uint8_t) ((color >> 8) & 0xFF);
    ...
    if (fw < 1) fw = 1;
    NspireRenderer_drawLine(&ner->fb,
        engine_to_fb_x(ner, x1), engine_to_fb_y(ner, y1),
        engine_to_fb_x(ner, x2), engine_to_fb_y(ner, y2),
        fw, nspire_rgb565(rr, g, b),
        alpha >= 0.996f ? 255 : (uint8_t) (alpha * 255.0f));
}
static void ns_drawLineColor(Renderer* r, float x1, float y1, float x2, float y2, float width, uint32_t c1, uint32_t c2, float alpha) {
    (void) c2; // no gradient: use the start color
    ns_drawLine(r, x1, y1, x2, y2, width, c1, alpha);
}
static void ns_drawTriangle(Renderer* r, float x1, float y1, float x2, float y2, float x3, float y3, bool outline) {
    NspireEngineRenderer* ner = as_ner(r);
    if (r->drawAlpha <= 0.0f) return;
...
// Pick a draw color from the engine's drawColor (BGR) so engine-side font tints come through.
...
// Render text using the GameMaker font selected by r->drawFont. Each glyph is a sub-rect of
// the font's TPAG on a cooked atlas; we halve the engine-space source coords (preprocessor
// halved the atlas pixels), then blit at the engine-space dest scaled to halved framebuffer
// space. Falls back to the 5x7 overlay font if the font is missing/invalid.
static void render_game_text(NspireEngineRenderer* ner, const char* text, float x, float y, float xs, float ys, uint16_t fallbackColor) {
    if (!text || !ner->base.dataWin) {
        NspireOverlay_drawTextScaled(&ner->fb, engine_to_fb_x(ner, x), engine_to_fb_y(ner, y), fallbackColor, 1, text);
        return;
    }
    ...
    if (atlas->bpp == 8) {
        if (t->clutIndex < (uint16_t) ner->assets->clut8Count) palette = ner->assets->clut8[t->clutIndex].entries;
    } else {
        if (t->clutIndex < (uint16_t) ner->assets->clut4Count) palette = ner->assets->clut4[t->clutIndex].entries;
    }
    if (!palette) {
        NspireOverlay_drawTextScaled(&ner->fb, engine_to_fb_x(ner, x), engine_to_fb_y(ner, y), fallbackColor, 1, text);
        return;
    ...
    // Fonts skip the preprocessor's halving pass — atlas is at full engine resolution.
    // Engine's full text-scale formula (from gs_renderer.c gsResolveFontState):
    //   screenScale = xscale * font->scaleX * gs->scaleX
    // For us, viewToFb already folds in the gs->scaleX equivalent (it's the
    // engine-to-framebuffer ratio). So effective glyph-to-fb scale = xs * fontScale * viewToFb.
    ...
    // No floor clamp on effScale. The earlier clamp-to-1.0 was a workaround for what
    // looked like "downsampling destroys pixel fonts" — but that garble was actually the
    // 512-atlas oversize-resize corrupting fnt_main (now fixed in the cook). With correct
    // full-res glyph data, the area-pool Shrunk path renders text at the engine's intended
    // size (battle's 640-wide GUI → effScale~0.5 → proper half-size text that fits its box).
    ...
    for (const char* p = text; *p; p++) {
        ...
        if (srcW > 0 && srcH > 0) {
            // Per-glyph X offset (engine: cursorX + glyph->offset). Many fonts use this
            // to position narrow chars (e.g. 'i', 'l') correctly within their cell.
            int32_t destX = penFbX + (int32_t) ((float) g->offset * effScaleX);
            int32_t destY = penFbY;
            if (effScaleX > 0.999f && effScaleX < 1.001f && effScaleY > 0.999f && effScaleY < 1.001f) {
                // Native size — no rescale (effScale ~= 1).
                if (useTint) {
                    if (atlas->bpp == 8) {
                        NspireRenderer_drawGlyphPart8Solid(&ner->fb, atlas->pixels, atlas->stride,
                            srcX, srcY, srcW, srcH, fallbackColor, destX, destY);
                    } else {
                        NspireRenderer_drawGlyphPart4Solid(&ner->fb, atlas->pixels, atlas->stride,
                            srcX, srcY, srcW, srcH, fallbackColor, destX, destY);
                    }
                } else if (atlas->bpp == 8) {
                    NspireRenderer_drawSpritePart8(&ner->fb, atlas->pixels, atlas->stride,
                ...
                if (useTint) {
                    if (atlas->bpp == 8) {
                        NspireRenderer_drawGlyphPart8ShrunkSolid(&ner->fb, atlas->pixels, atlas->stride,
                            srcX, srcY, srcW, srcH, fallbackColor, destX, destY, destW, destH);
                    } else {
                        NspireRenderer_drawGlyphPart4ShrunkSolid(&ner->fb, atlas->pixels, atlas->stride,
                            srcX, srcY, srcW, srcH, fallbackColor, destX, destY, destW, destH);
                    }
                } else if (atlas->bpp == 8) {
                    NspireRenderer_drawGlyphPart8Shrunk(&ner->fb, atlas->pixels, atlas->stride,
                        srcX, srcY, srcW, srcH, palette, destX, destY, destW, destH);
                } else {
                    NspireRenderer_drawGlyphPart4Shrunk(&ner->fb, atlas->pixels, atlas->stride,
                        srcX, srcY, srcW, srcH, palette, destX, destY, destW, destH);
                }
            } else {
...
static void ns_drawText(Renderer* r, const char* text, float x, float y, float xs, float ys, float angleDeg) {
    NspireEngineRenderer* ner = as_ner(r);
    (void) angleDeg;
    if (!text) return;
...
static void ns_drawTextColor(Renderer* r, const char* text, float x, float y, float xs, float ys, float angleDeg, int32_t c1, int32_t c2, int32_t c3, int32_t c4, float alpha) {
    (void) c2; (void) c3; (void) c4; (void) alpha; (void) angleDeg;
    NspireEngineRenderer* ner = as_ner(r);
    if (!text) return;
...
static int32_t ns_createSpriteFromSurface(Renderer* r, int32_t s, int32_t x, int32_t y, int32_t w, int32_t h, bool rb, bool sm, int32_t xo, int32_t yo) {
    (void) r; (void) s; (void) x; (void) y; (void) w; (void) h; (void) rb; (void) sm; (void) xo; (void) yo;
    return -1;
}
static void ns_deleteSprite(Renderer* r, int32_t s) { (void) r; (void) s; }

static void ns_gpuSetBlendMode(Renderer* r, int32_t mode) { (void) r; (void) mode; }
static void ns_gpuSetBlendModeExt(Renderer* r, int32_t s, int32_t d) { (void) r; (void) s; (void) d; }
static void ns_gpuSetBlendEnable(Renderer* r, bool e) { (void) r; (void) e; }
static void ns_gpuSetAlphaTestEnable(Renderer* r, bool e) { (void) r; (void) e; }
static void ns_gpuSetAlphaTestRef(Renderer* r, uint8_t v) { (void) r; (void) v; }
static void ns_gpuSetColorWriteEnable(Renderer* r, bool a, bool b, bool c, bool d) { (void) r; (void) a; (void) b; (void) c; (void) d; }
...
static void ns_gpuGetColorWriteEnable(Renderer* r, bool* red, bool* green, bool* blue, bool* alpha) {
    (void) r;
    if (red)   *red   = true;
    if (green) *green = true;
    if (blue)  *blue  = true;
...
static bool ns_gpuGetBlendEnable(Renderer* r) { (void) r; return true; }
static void ns_gpuSetFog(Renderer* r, bool e, uint32_t c) { (void) r; (void) e; (void) c; }
...
static void ns_bindTarget(NspireEngineRenderer* ner, int32_t id) {
    bool toSurface  = (id >= 0 && id < NS_MAX_SURFACES && ner->surfaces[id].px != NULL);
    ...
    if (toSurface) {
        ...
        ner->viewOriginX = 0.0f;    ner->viewOriginY = 0.0f;
        ner->viewToFbScaleX = 1.0f; ner->viewToFbScaleY = 1.0f;
        ner->portFbX = 0;           ner->portFbY = 0;
        ner->fb.scissorX = 0;       ner->fb.scissorY = 0;
...
static int32_t ns_createSurface(Renderer* r, int32_t w, int32_t h) {
    ...
    for (int32_t i = 0; i < NS_MAX_SURFACES; i++) {
        if (!ner->surfaces[i].px) {
            ...
            if (!p) return -1;   // OOM -> engine degrades (surface_exists false)
...
#if !NS_ENABLE_SURFACES
    (void) r; (void) s; return false;
#else
    NspireEngineRenderer* ner = as_ner(r);
    if (s == APPLICATION_SURFACE_ID) return true;
...
static float ns_getSurfaceWidth(Renderer* r, int32_t s) {
    NspireEngineRenderer* ner = as_ner(r);
    if (s == APPLICATION_SURFACE_ID) return (float) ner->screenW;
    if (s >= 0 && s < NS_MAX_SURFACES && ner->surfaces[s].px) return (float) ner->surfaces[s].w;
    return 0.0f;
}
static float ns_getSurfaceHeight(Renderer* r, int32_t s) {
    NspireEngineRenderer* ner = as_ner(r);
    if (s == APPLICATION_SURFACE_ID) return (float) ner->screenH;
    if (s >= 0 && s < NS_MAX_SURFACES && ner->surfaces[s].px) return (float) ner->surfaces[s].h;
    return 0.0f;
}
// Surfaces are not atlas-halved, so view->fb factor is viewToFb / HR_SCALE.
static inline float ns_v2f_x(NspireEngineRenderer* ner) { return ner->viewToFbScaleX / HR_SCALE_ATLAS; }
static inline float ns_v2f_y(NspireEngineRenderer* ner) { return ner->viewToFbScaleY / HR_SCALE_ATLAS; }
...
static void ns_drawSurface(Renderer* r, int32_t s, int32_t srcLeft, int32_t srcTop,
                           int32_t srcW, int32_t srcH, float x, float y,
                           float xs, float ys, float ang, uint32_t c, float a) {
    NspireEngineRenderer* ner = as_ner(r);
    (void) ang;
    if (s < 0 || s >= NS_MAX_SURFACES || !ner->surfaces[s].px || a <= 0.0f) return;
    ...
    if (srcW <= 0 || srcW > SW - srcLeft) srcW = SW - srcLeft;
    if (srcH <= 0 || srcH > SH - srcTop)  srcH = SH - srcTop;
    ...
    NspireRenderer_blitSurface(&ner->fb, ner->surfaces[s].px, SW,
        srcLeft, srcTop, srcW, srcH,
        engine_to_fb_x(ner, x), engine_to_fb_y(ner, y), dw, dh, a8, c);
}
static void ns_surfaceResize(Renderer* r, int32_t s, int32_t w, int32_t h) {
    NspireEngineRenderer* ner = as_ner(r);
    if (s < 0 || s >= NS_MAX_SURFACES || !ner->surfaces[s].px || w <= 0 || h <= 0) return;
    uint16_t* p = (uint16_t*) malloc((size_t) w * (size_t) h * sizeof(uint16_t));
    if (!p) return;
    memset(p, 0, (size_t) w * (size_t) h * sizeof(uint16_t));
    ...
    ner->surfaces[s].w = w;
    ner->surfaces[s].h = h;
    if (ner->activeSurface == s) ns_bindTarget(ner, s);   // refresh bound dims
}
static void ns_surfaceFree(Renderer* r, int32_t s) {
    NspireEngineRenderer* ner = as_ner(r);
    if (s < 0 || s >= NS_MAX_SURFACES || !ner->surfaces[s].px) return;
    free(ner->surfaces[s].px);
    ner->surfaces[s].px = NULL;
    ner->surfaces[s].w = ner->surfaces[s].h = 0;
    if (ner->activeSurface == s) ns_bindTarget(ner, APPLICATION_SURFACE_ID);
}
static void ns_surfaceCopy(Renderer* r, int32_t d, int32_t dx, int32_t dy, int32_t s, int32_t sx, int32_t sy, int32_t sw, int32_t sh, bool p) {
    NspireEngineRenderer* ner = as_ner(r);
    (void) p;
    if (d < 0 || d >= NS_MAX_SURFACES || !ner->surfaces[d].px) return;
    if (s < 0 || s >= NS_MAX_SURFACES || !ner->surfaces[s].px) return;
    int32_t SW = ner->surfaces[s].w, SH = ner->surfaces[s].h;
    if (sx < 0) sx = 0;
    if (sy < 0) sy = 0;
    if (sw <= 0 || sw > SW - sx) sw = SW - sx;
    if (sh <= 0 || sh > SH - sy) sh = SH - sy;
    if (sw <= 0 || sh <= 0) return;
    NspireRenderer tmp;
    NspireRenderer_init(&tmp, ner->surfaces[d].px, ner->surfaces[d].w, ner->surfaces[d].h);
    NspireRenderer_blitSurface(&tmp, ner->surfaces[s].px, SW, sx, sy, sw, sh,
        dx, dy, sw, sh, 255, 0xFFFFFFu);
}
static bool ns_surfaceGetPixels(Renderer* r, int32_t s, uint8_t* out) {
    NspireEngineRenderer* ner = as_ner(r);
    if (!out || s < 0 || s >= NS_MAX_SURFACES || !ner->surfaces[s].px) return false;
...
Renderer* NspireEngineRenderer_create(NspireAssets* assets, uint16_t* framebuffer, int32_t fbW, int32_t fbH) {
    NspireEngineRenderer* ner = (NspireEngineRenderer*) calloc(1, sizeof(NspireEngineRenderer));
    if (!ner) return NULL;
    ...
    ner->assets = assets;
    // Sensible defaults so any draw call before the first beginView still maps coords.
...
void NspireEngineRenderer_destroy(Renderer* r) {
    NspireEngineRenderer* ner = as_ner(r);
    for (int32_t i = 0; i < NS_MAX_SURFACES; i++) {
        if (ner->surfaces[i].px) free(ner->surfaces[i].px);
    }
    free(r);
}
