...
static void parseSPRT(BinaryReader* reader, DataWin* dw, bool skipLoadingPreciseMasksForNonPreciseSprites, bool skipAllSpriteMasks, void (*log)(const char*, void*), void* logUserData) {
    Sprt* s = &dw->sprt;
    uint32_t count;
    uint32_t* ptrs = readPointerTable(reader, &count);
    s->count = count;
    ...
    if (log) { char buf[64]; snprintf(buf, sizeof(buf), "SPRT: parsing %u sprites...", count); log(buf, logUserData); }
    repeat(count, i) {
        BinaryReader_seek(reader, ptrs[i]);
        ...
        if (maskDataCount > 0 && spr->width > 0 && spr->height > 0) {
            ...
            if (skipAllSpriteMasks) {
                BinaryReader_skip(reader, bytesPerMask * maskDataCount);
                spr->masks = nullptr;
            } else if (spr->sepMasks == 1 || !skipLoadingPreciseMasksForNonPreciseSprites) {
...
// Returns true if the caller's chunkBuffer ownership was transferred to dw->bytecodeBuffer
// (caller must NOT free it). Returns false in streaming mode (no chunkBuffer present)
// or when a separate bytecode allocation was made.
static bool parseCODE(BinaryReader* reader, DataWin* dw, uint32_t chunkLength, size_t chunkDataStart) {
    Code* c = &dw->code;
    ...
    if (codeCount == 0) { free(codePtrs); c->entries = nullptr; return false; }
    ...
    // If we have a bulk-loaded chunk buffer the bytecode blob is already in memory inside it.
    // Repurpose the whole chunk buffer as the bytecodeBuffer (wastes the pointer-table prefix,
    // a few KB, but avoids a duplicate ~5 MB allocation that fragmentation usually defeats).
    if (reader->buffer != nullptr && reader->bufferBase == chunkDataStart && reader->bufferSize == chunkLength) {
        dw->bytecodeBufferBase = chunkDataStart;
        dw->bytecodeBuffer = reader->buffer;
        // Tell the parser main loop that we've taken over the chunk buffer so it isn't freed.
        BinaryReader_clearBuffer(reader);
        (void) blobStart; (void) blobSize;
        return true;
    }
...
DataWin* DataWin_parse(const char* filePath, DataWinParserOptions options) {
    ...
    while ((size_t) fileSize > BinaryReader_getPosition(&reader)) {
        ...
        if (options.progressCallback) {
            options.progressCallback(chunkName, chunkIndex, totalChunks, chunkLength, dw, options.progressCallbackUserData);
        }
        ...
        // Bulk-read the chunk data into memory for fast parsing — but skip the bulk read if the
        // chunk is larger than streamChunkThresholdBytes (parser falls back to file-mode reads).
        // Loop fread calls — some libcs (e.g. newlib on Ndless) can short-read on multi-MB transfers
        // while returning success; verify total against expected and retry from the last good byte.
        uint8_t* chunkBuffer = nullptr;
        bool streamThisChunk = (options.streamChunkThresholdBytes > 0 && chunkLength > options.streamChunkThresholdBytes);
        if (shouldParse && chunkLength > 0 && !streamThisChunk) {
            chunkBuffer = safeMalloc(chunkLength);
            size_t total = 0;
            while (total < chunkLength) {
                size_t got = fread(chunkBuffer + total, 1, chunkLength - total, reader.file);
                if (got == 0) {
                    fprintf(stderr, "DataWin: short read on chunk %.4s (expected %u, got %zu, errno=%d eof=%d)\n",
                            chunkName, chunkLength, total, errno, feof(reader.file));
                    exit(1);
                }
                total += got;
            }
            BinaryReader_setBuffer(&reader, chunkBuffer, chunkDataStart, chunkLength);
            if (options.chunkLoadedCallback) {
                options.chunkLoadedCallback(chunkName, chunkLength, options.progressCallbackUserData);
            }
        }
        ...
        if (options.parseGen8 && memcmp(chunkName, "GEN8", 4) == 0) {
            ...
            parseSPRT(&reader, dw, options.skipLoadingPreciseMasksForNonPreciseSprites, options.skipAllSpriteMasks, options.parseDebugLog, options.progressCallbackUserData);
            ...
            if (parseCODE(&reader, dw, chunkLength, chunkDataStart)) {
                // parseCODE took ownership of chunkBuffer (it's now dw->bytecodeBuffer).
                chunkBuffer = nullptr;
            }
