...
typedef struct {
    ...
    // If true, ALL sprite collision mask payloads are skipped (even sepMasks==1 sprites get spr->masks=nullptr).
    // Runtime callers (collision.h, vm_builtins.c) already null-check and fall back to bbox collision when masks
    // are absent. Useful on memory-constrained targets where the per-frame mask bitmaps would exhaust the heap.
    bool skipAllSpriteMasks;

    // If > 0, chunks whose payload is larger than this threshold (in bytes) are parsed directly from
    // the file instead of being bulk-loaded into RAM. Slower (more fread calls, more seeks) but avoids
    // requiring a single contiguous allocation for a multi-megabyte chunk. Important on heap-constrained
    // platforms where fragmentation prevents large mallocs even when total free memory is sufficient.
    uint32_t streamChunkThresholdBytes;
    ...
    // chunkLength: byte length of this chunk's payload (about to be malloc'd if parsed)
    ...
    void (*progressCallback)(const char* chunkName, int chunkIndex, int totalChunks, uint32_t chunkLength, DataWin* dataWin, void* userData);
    void* progressCallbackUserData;

    // Optional callback fired AFTER a chunk has been bulk-loaded into memory but BEFORE it is parsed.
    // Lets diagnostic builds distinguish "malloc/fread failed" from "parse* crashed". Skipped chunks
    // (parseXxx flag is false) do not trigger this callback. userData passed through unchanged.
    void (*chunkLoadedCallback)(const char* chunkName, uint32_t chunkLength, void* userData);

    // Optional fine-grained debug log fired from inside chunk parse functions for diagnostic builds.
    // Calls are rare (a few per chunk) but help localize hangs/aborts to a specific stage.
    void (*parseDebugLog)(const char* msg, void* userData);
